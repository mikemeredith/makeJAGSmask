...
test_that("convertOutput gives same output",  {
  data(exampleData)
  secrmask <- secr::make.mask(traps, spacing=2000, type='polygon',
    poly=patch, cell='any')
  mymask <- convertMask(secrmask, traps, plot=FALSE)

  res <- convertOutput(JAGSoutput, JAGSmask = mymask)
  expect_is(res, "array")
  expect_equal(dim(res), dim(JAGSoutput))
  expect_equivalent(sum(is.na(res)), sum(is.na(JAGSoutput)))
  expect_equivalent(round(mean(res, na.rm=TRUE)), 466431)

  # list input
  inp <- list(y = JAGSoutput[, , 2],
              x = JAGSoutput[, , 1])  # needs new version of convertOutput
